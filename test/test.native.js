...
tape( 'main export is a function', opts, function test( t ) {
	...
	t.strictEqual( typeof smaxsorted, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 1', opts, function test( t ) {
	t.strictEqual( smaxsorted.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the maximum value of a one-dimensional ndarray', opts, function test( t ) {
	var x;
	var v;

	x = new Float32Array( [ -3.0, -2.0, -1.0, 1.0, 2.0, 3.0 ] );
	v = smaxsorted( [ vector( x, 6, 1, 0 ) ] );
